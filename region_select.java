        ...
        b_apply.setOnClickListener(new Button.OnClickListener() {                   //  적용 버튼을 누르면
            public void onClick(View v) {                                          //  번들에 데이터(이름, 코드)를 담아 finish
                if (checkcount_first == 0){
                    Toast.makeText(region_select.this, "1차지역을 선택해주세요", Toast.LENGTH_LONG).show();
                }
                else if (checkcount_second == 0 || parameter_name == null){
                    Toast.makeText(getApplicationContext(), "2차지역을 선택해주세요", Toast.LENGTH_LONG).show();
                }
                else {
